...
export class TaskComponent implements OnDestroy {
  ...
  private tagsSupscription?: any;
  private taskSupscription?: any;
  private usersSupscription?: any;
  ...
  ngOnInit() {
    this.tagsSupscription = this.tagService.getAll().subscribe((result) => {
      // Lógica adicional después de crear la tarea, si es necesario
      this.allTags = result;
    });

    this.usersSupscription = this.userService.getAll().subscribe((data) => {
      this.allUsers = data;
    });
  ...
  ngOnDestroy(): void {
    this.tagsSupscription.unsubscribe();
    this.usersSupscription.unsubscribe();
  }
  ...
  addTags(){
    this.taskSupscription = this.taskService.addTagsToTask(this.task?.id, this.tags).subscribe(() => {
      // Lógica adicional después de crear la tarea, si es necesario
      this.edit = false;
    });
    this.tagsUpdated.emit(this.tags);
  ...
  deleteTask(): void {
    ...
    // Emitir evento de borrado hacia el componente padre
